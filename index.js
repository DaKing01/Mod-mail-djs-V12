...
client.on("-----------------ready-----------------", () => {
console.log("I am ready to receive Mail :D")
...
client.on("message", async message => {
  ...
  if(message.guild) {
      if(command == "setup") {
          ...
          return message.channel.send("Setup is Completed! Made by !Glitched!#6420 :D")
        ...
        if(message.channel.parentID == message.guild.channels.cache.find((x) => x.name == "MODMAIL").id) {
            ...
            if(!person) {
                return message.channel.send("I am Unable to close the channel and this error is coming because probaly channel name is changed or Deleted.")
            }
  ...
  if(!message.guild) {
    ...
    if(command === 'info') {
    msg.react("😀")
    msg.reply('PARAGON BOT, MADE BY !Glitched!#6420!')
    console.log('-----------------------------------------------------------------')
  }
  ...
  if(command === 'ping') {
    ...
    msg.channel.send('Latency low  ✅')
    msg.channel.send('Latency mid  ❌')
    msg.channel.send('Latency high ❌')
    console.log('-----------------------------------------------------------------')
  ...
  if(command === 'help') {
    msg.react(":wrench: ")
    msg.channel.send(':hindu_temple: ---------COMMANDS---------:hindu_temple: ')
    msg.channel.send(':wrench: Info      [✅] ')
    msg.channel.send(':wrench: Kick      [✅] ')
    msg.channel.send(':wrench: Ban       [❌] ')
    msg.channel.send(':wrench: Clear     [✅] ')
    msg.channel.send(' [✅]= WORKING! [❌]= UNDER DEV OR NOT WORKING! ')
    console.log('-----------------------------------------------------------------')
  }


  if (command === "clear") {
    if(!msg.member.roles.cache.has('759906032578461736')) {
      msg.reply('you dont have permission to clear chat')
      ...
      msg.channel.send(`Deleted  ${args[0]} posts`);
       console.log('-----------------------------------------------------------------')
 ...
 if(command === 'kick') {
   //verify that user has moderation role
    if(!msg.member.roles.cache.has('759906031475621891')) {
  ...
  if(command === 'ban') {
   //verify that user has moderation role
    if(!msg.member.roles.cache.has('759906031475621891')) {
      msg.reply('you dont have permission to ban users')
      // if user doesnt have the role, we return without kicking the user
      return
    }
    //check to make sure a user was actually mentioned, if not we return because bot doesnt know who to kick
